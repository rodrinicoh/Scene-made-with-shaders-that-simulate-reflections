    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 fragmentShader(v2f i) : SV_Target
            {
                ...
                float3 aux =cookTorrance*_LightIntensity_d;//POR QUE IBA MULTIPLICADO POR EL LIGHT INTENSITY ACA?
                

                //difuso
                float3 diffuse = 0;
                diffuse = 1*_LightIntensity_d*NdotL*_MaterialKd;
                fixed4 fragColor= fixed4(diffuse+aux,1)*_MaterialColor;
                return fragColor;
            }
            ENDCG
        }
