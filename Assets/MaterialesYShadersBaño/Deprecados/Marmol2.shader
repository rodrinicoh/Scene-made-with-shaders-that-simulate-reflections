Shader "MarmolProcedural2"
{
	Properties
	{
		_ku("ku", Range(1,500)) = 30
		_kv("kv", Range(1,500)) = 30
		...
		SubShader
	{
		Pass
		{
			...
			float2 random2(float2 st) {
				st = float2(dot(st, float2(269.5, 183.3)),dot(st, float2(127.1, 311.7)));
				return -1.0 + 2.0 * frac(sin(st) * 437580.5453123);
			}
			...
			fixed4 frag(v2f i) : SV_Target
			{
                ...
                float f = perlinNoise(_Density * i.uv);

                f = 0.5 + 0.5 * f; //LO LLEVA al rango 0-1 (antes estaba en -1, 1)

                float c=marble(i.uv.x,i.uv.y)*f;
